...
namespace DatabaseEditor.Dbc
{
    ...
    {
        static MethodInfo BinaryReader_ReadInt32       = typeof(BinaryReader).GetMethod("ReadInt32");
        static MethodInfo BinaryReader_ReadSingle      = typeof(BinaryReader).GetMethod("ReadSingle");
        static MethodInfo DbcTable_GetString           = typeof(DbcTable).GetMethod("GetString");
        static ConstructorInfo DbcStringReference_ctor = typeof(DbcStringReference).GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null, new Type[] { typeof(DbcTable), typeof(int) }, null);

        internal static DbcReaderProducer<T> Compile<T>() where T : class
        {
        ...
        static void EmitForProperty(TargetInfo info, ILGenerator generator)
        {
        ...
        static void EmitTypeData(TargetInfo info, ILGenerator generator)
        {
            switch (info.Type)
            {
        ...
        static void EmitForField(TargetInfo info, ILGenerator generator)
        {
            Debug.Assert(info != null);
            Debug.Assert(generator != null);
        ...
        internal class TargetInfo
        {
            ...
            public void SetValue<TValue, TTarget>(TTarget target, TValue inputVal) where TTarget : class
            {
                if (Property != null)
